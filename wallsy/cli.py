...
Create beautiful images, effects, and desktop wallpapers through composable edits on the
command line.

This module defines the entry point to the wallsy CLI. It defines a 'cli' command
group which collects input streams from standard input as well as any provided global
options.

A callback processor is passed a list of callbacks for each of the subcommands that are
invoked. Each callback accepts an input stream and wraps the stream in a new generator
that, when later accessed, yields values from the input stream after having been
processed by some image processing function defined internally.

After all of the callbacks have executed, we are left with a final "stream"
generator-in-generator that can be iterated over to trigger the image processing
...
def cli(ctx: click.Context, files, urls, verbosity):
    ...
    create beautiful images, effects, and desktop wallpapers through composable
    edits on the command line.
    ...
    Wallsy is designed to chain commands together into powerful one-line expressions to
    collect, edit, and compose images with a focus on use in personal applications like
    wallpapers, background images for streaming/creative applications, etc.
    ...
    desktop image as input, e.g.
        ...
        use your current wallpaper as input for some cool effects then
        save it back to your wallpaper
    ...
    To see what's available and for detailed help text
    add --help to the specified command, e.g.
    ...
    Support receiving a file path from standard input instead of as a command line
    argument. This is really cool and delves a bit deep into the Python standard
    library. The OS module allows us to query a file descriptor and return information
    from the operating system about that file, such as ownership, size, etc. We can use
    the stat module to deconstruct and query the resulting stat object that os.stat
    provides, and in our case use the IS_FIFO method to determine if the file
    descriptor is a UNIX pipe or not. Major kudos to the following SO article and the
    ...
    stream = (file for file in streams)
